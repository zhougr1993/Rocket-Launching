...
def main():
    ...
    def h(sample):
        ...
        if opt.teacher_id != '':
            ...
            ys_s = F.softmax(ys / opt.temperature)
            y_s = F.softmax(y / opt.temperature)
            ...
            #loss_course = opt.beta * (y*(torch.log(y)-torch.log(ys)) + ys*(torch.log(ys) - torch.log(y))).sum()/opt.batchSize
            ...
            return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + opt.beta * (y_tech_temp*(-torch.log(torch.clamp(ys_s,0.00001,1))) + ys_s*(torch.log(torch.clamp(ys_s,0.00001,1)) - torch.log(torch.clamp(y_tech_temp,0.00001,1)))).sum()/opt.batchSize, (ys, y, loss_student, loss_teacher, loss_course)
