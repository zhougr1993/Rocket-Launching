...
def create_dataset(opt, mode):
    ...
    if mode == 'train':
		...
		modebool = False
    return ds.transform({0: train_transform if modebool else convert})
...
def resnet(stu_depth, depth, width, num_classes):
    ...
    def group(o, params, stats, base, mode, stride):
        for i in range(n):
            o = block(o, params, stats, '%s.block%d'%(base,i), mode, stride if i==0 else 1, 't', i>=2 )
            # print 'teach : %s.block%d'%(base,i)
        return o
    
    def group_student(o, params, stats, base, mode, stride):
        for i in range(n_s):
            o = block(o, params, stats, '%s.block%d'%(base,i), mode, stride if i==0 else 1, 's', False)
