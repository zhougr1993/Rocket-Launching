...
parser.add_argument('--student_depth', default=16, type=int)
...
def resnet_origin(depth, width, num_classes):
...
def resnet(stu_depth, depth, width, num_classes):
    assert (depth - 4) % 6 == 0, 'depth should be 6n+4'
    n = (depth - 4) // 6
    ...
    def block(x, params, stats, base, mode, stride, flag):
        if flag == 's':
    ...
    def group(o, params, stats, base, mode, stride):
        for i in range(n):
            o = block(o, params, stats, '%s.block%d'%(base,i), mode, stride if i==0 else 1, 't')
            # print 'teach : %s.block%d'%(base,i)
        return o
    
    def group_student(o, params, stats, base, mode, stride):
        for i in range(n_s):
            o = block(o, params, stats, '%s.block%d'%(base,i), mode, stride if i==0 else 1, 's')
            # print 'student : %s.block%d'%(base,i)
        return o
    ...
    def f(input, params, stats, mode, prefix=''):
        ...
        gs0 = group_student(x, params, stats, prefix+'group0', mode, 1)
        gs1 = group_student(gs0, params, stats, prefix+'group1', mode, 2)
        gs2 = group_student(gs1, params, stats, prefix+'group2', mode, 2)
...
def main():
    ...
    f_s, params_s, stats_s = resnet(opt.student_depth, opt.depth, opt.width, num_classes)

    # deal with teacher
    if opt.teacher_id != '':
        with open(os.path.join('experiment_log', opt.teacher_id, 'log.txt'), 'r') as ff:
            line = ff.readline()
            r = line.find('json_stats')
            info = json.loads(line[r+12:])
        f_t = resnet_origin(info['depth'], info['width'], num_classes)[0]
        model_data = torch.load(os.path.join('experiment_log', opt.teacher_id, 'model.pt7'))
    ...
    def h(sample):
        ...
        if opt.teacher_id != '':
            ys, y_t_auto, y_t = data_parallel(f, inputs, params, stats, sample[2], np.arange(opt.ngpu))[:3]
	    loss_l2 = torch.nn.MSELoss()
	    T = 4
            loss_student = F.cross_entropy(ys, targets)
            loss_teacher = F.cross_entropy(y_t_auto, targets)
            y_s = F.softmax(y_t_auto)
            ys_s = F.softmax(ys)
            loss_course = opt.beta * (y_s*(torch.log(torch.clamp(y_s,0.00001,1))-torch.log(torch.clamp(ys_s,0.00001,1))) +  ys_s*(torch.log(torch.clamp(ys_s,0.00001,1)) - torch.log(torch.clamp(y_s,0.00001,1)))).sum()/opt.batchSize
            y_tech_temp = torch.autograd.Variable(y_s.data, requires_grad = False)
	    log_kd = distillation(ys, y_t, targets, opt.temperature, opt.alpha)  
            return distillation(ys, y_t, targets, opt.temperature, opt.alpha) \
                    + F.cross_entropy(y_t_auto, targets) + F.cross_entropy(ys, targets) + opt.beta * (y_s*(torch.log(torch.clamp(y_s,0.00001,1))-torch.log(torch.clamp(ys_s,0.00001,1))) +  ys_s*(torch.log(torch.clamp(ys_s,0.00001,1)) - torch.log(torch.clamp(y_s,0.00001,1)))).sum()/opt.batchSize , (ys, y_t_auto, loss_student, loss_teacher, loss_course, log_kd)
            ...
            T = 4
            # CONST_STEP_FLAG += 1
            # epoch_scale = math.log(CONST_STEP_FLAG + 1)
            # print "KL : ", F.kl_div(F.log_softmax(y/T), F.softmax(ys/T)) * (T*T * 2. * 0.9)
            # print "cross entropy : ",F.cross_entropy(y, targets)
            # print epoch_scale
            loss_student = F.cross_entropy(ys, targets)
            loss_teacher = F.cross_entropy(y, targets)
            loss_course = opt.beta * ((y - ys)*(y - ys)).sum()/opt.batchSize
            # print opt.running_sigma
            # return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + F.binary_cross_entropy(F.sigmoid(y), F.sigmoid(ys)), ys
            # return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + epoch_scale * opt.beta * ((y - ys)*(y - ys)).sum()/opt.batchSize, ys
            # return (3 - 100 * opt.running_sigma) * F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + opt.running_sigma * ((y - ys)*(y - ys)).sum()/opt.batchSize, (ys, y, loss_student, loss_teacher, loss_course)
            y_tech_temp = torch.autograd.Variable(y.data, requires_grad = False)
            return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + opt.beta * ((y_tech_temp - ys)*(y_tech_temp - ys)).sum()/opt.batchSize, (ys, y, loss_student, loss_teacher, loss_course)
            # return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + F.kl_div(F.log_softmax(y/T), F.softmax(ys/T)) * (T*T * 2. * 0.9), ys
    ...
    def on_forward(state):
        ...
        meter_loss_d.add(state['output'][5].data[0])
    ...
    def on_end_epoch(state):
        ...
        train_loss_d = meter_loss_d.value()
        ...
        print log({
            ...
            "test_loss_kd": meter_loss_d.value()[0],
