...
def main():
    ...
    def h(sample):
	    ...
	    T = 4
            ...
            loss_course = opt.beta * ((y_t_auto - ys)*(y_t_auto - ys)).sum()/opt.batchSize
	    ...
	    log_kd = distillation(ys, y_t, targets, opt.temperature, opt.alpha)  
            return distillation(ys, y_t, targets, opt.temperature, opt.alpha) \
                    + F.cross_entropy(y_t_auto, targets) + F.cross_entropy(ys, targets) + opt.beta * ((y_tech_temp - ys)*(y_tech_temp - ys)).sum()/opt.batchSize, (ys, y_t_auto, loss_student, loss_teacher, loss_course, log_kd)
        else:
