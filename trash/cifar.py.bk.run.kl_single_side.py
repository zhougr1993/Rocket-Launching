...
def main():
    ...
    def h(sample):
        ...
        if opt.teacher_id != '':
            ...
            loss_course = opt.beta * (y*(torch.log(y)-torch.log(ys))).sum()/opt.batchSize
            ...
            return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + opt.beta * (y*(torch.log(y)-torch.log(ys)) ).sum()/opt.batchSize, (ys, y, loss_student, loss_teacher, loss_course)
