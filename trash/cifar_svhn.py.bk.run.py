...
parser.add_argument('--data_root', default='./svhn/', type=str)
...
def create_dataset(opt, mode, phase):
    if opt.dataset in ['CIFAR10', 'CIFAR100']:
        ...
        from svhn_reader import SVHN, random_split_svhn
        convert = tnt.transform.compose([
            lambda x: x.astype(np.float32),
            lambda x: x.transpose(2,0,1).astype(np.float32),
            torch.from_numpy,
        ...
        # random_split_svhn(opt.data_root)
        ds = SVHN(opt.data_root, split=phase)
        ds = tnt.dataset.TensorDataset([
            getattr(ds, 'data'),
            getattr(ds, 'labels')])
...
def main():
    ...
    num_classes = 10 if opt.dataset in ['CIFAR10', 'SVHN'] else 100
    ...
    def create_iterator(mode, phase):
        ds = create_dataset(opt, mode, phase)
        return ds.parallel(batch_size=opt.batchSize, shuffle=mode,
                           num_workers=opt.nthread, pin_memory=True)

    if opt.dataset in ['CIFAR10', 'CIFAR100']:
        train_loader = create_iterator(True, 'train')
        test_loader = create_iterator(False, 'test')
    elif opt.dataset == 'SVHN':
        train_loader = create_iterator(True, 'train')
        valid_loader = create_iterator(False, 'valid')
        test_loader = create_iterator(False, 'test')
    else:
        raise ValueError('unsupported data set')
    ...
    def on_end_epoch(state):
        ...
        valid_loss = meter_loss.valud()[0]
        valid_loss_s = meter_loss_s.value()[0]
        valid_loss_t = meter_loss_t.value()[0]
        valid_loss_c = meter_loss_c.value()[0]
        valid_acc_s = classacc_s.value()[0]
        valid_acc_t = classacc_t.value()[0]
        valid_time = timer_test.value()
        ...
        print log({
            ...
            "valid_loss_student": valid_loss_s,
            "valid_loss_teacher": valid_loss_t,
            "valid_loss_course": valid_loss_c,
            "valid_acc_student": valid_acc_s,
            "valid_acc_teacher": valid_acc_t,
        ...
        print '==> id: %s (%d/%d), valid_acc: %.2f, test_acc: \33[91m%.2f\033[0m' % \
                       (opt.save, state['epoch'], opt.epochs, valid_acc_s, test_acc_s)
