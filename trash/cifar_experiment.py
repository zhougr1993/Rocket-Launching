...
def resnet(stu_depth, depth, width, num_classes):
    ...
    """
    params_stu_new = {'conv0': conv_params(3,16,3),
              # 'conv0_s': conv_params(3,16,3),
              'group0': gen_group_params(16, widths[0], 6),
              'group1': gen_group_params(widths[0], widths[1], 6),
              'group2': gen_group_params(widths[1], widths[2], 1),
              'bns': bnparams(widths[1]),
              'fcs': linear_params(widths[1], num_classes),
             }

    params_stu_old = {'conv0': conv_params(3,16,3),
              # 'conv0_s': conv_params(3,16,3),
              'group0': gen_group_params(16, widths[0], 2),
              'group1': gen_group_params(widths[0], widths[1], 2),
              'group2': gen_group_params(widths[1], widths[2], 2),
              'bns': bnparams(widths[2]),
              'fcs': linear_params(widths[2], num_classes),
             }
    params_stu_new = flatten_params(params_stu_new)
    params_stu_old = flatten_params(params_stu_old)
    print sum(p.numel() for p in params_stu_new.values())
    print sum(p.numel() for p in params_stu_old.values())
    """
    ...
    def f(input, params, stats, mode, prefix=''):
        ...
        gs0 = group_student(x, params, stats, prefix+'group0', mode, 1, 6)
        gs1 = group_student(gs0, params, stats, prefix+'group1', mode, 2, 6)
#        gs2 = group_student(gs1, params, stats, prefix+'group2', mode, 2, 1)
#        os = F.relu(batch_norm(gs2, params, stats, prefix+'bns', mode))
        os = F.relu(batch_norm(gs1, params, stats, prefix+'bns', mode))
        os = F.avg_pool2d(os, 16, 1, 0)
        os = os.view(os.size(0), -1)
	#print os.size()
        #print params[prefix+'fcs.weight'].size()
