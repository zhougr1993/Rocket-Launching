...
def main():
    ...
    def h(sample):
        ...
        if opt.teacher_id != '':
            ...
            ys_s = F.softmax(ys)
            y_s = F.softmax(y)
            loss_course = opt.beta * ((y_s - ys_s)*(y_s - ys_s)).sum()/opt.batchSize
            ...
            return F.cross_entropy(y, targets) + F.cross_entropy(ys, targets) + opt.beta * ((y_tech_temp - ys_s)*(y_tech_temp - ys_s)).sum()/opt.batchSize, (ys, y, loss_student, loss_teacher, loss_course)
